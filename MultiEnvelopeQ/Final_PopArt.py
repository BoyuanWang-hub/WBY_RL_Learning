...
from PopArtQNet1 import PopArtEnvMoNoF
...
class PopArtMORL:
    ...
    def pop(self, update_mode = 'in-place'):
        update_mode = '123' ### 取消in-place模式更新 ###
        assert isinstance(self.net, PopArtEnvMoNoF)
        for ii in range(self.mu.shape[0]): ### 对每一行均执行上述操作 ###
            ...
            if update_mode == 'in-place':
                ...
                self.net.actor_out.weight[ii].data.mul_(relative_sigma)
                self.net.actor_out.bias[ii].data.mul_(relative_sigma).add_((self.mu[ii] - self.next_mu[ii]) / self.next_sigma[ii])
            else:
                ...
                self.net.actor_out.weight[ii].data = relative_sigma * self.net.critic_out.weight[ii].data
                self.net.actor_out.bias[ii].data = relative_sigma * self.net.critic_out.bias[ii].data + \
                                                    (self.mu[ii] - self.next_mu[ii]) / self.next_sigma[ii]

    def update_sigma_mu(self):
        self.mu, self.sigma = self.next_mu, self.next_sigma

    def normalize_y(self, target): ### 草了 之前写错了 ###
    ...
    def caculate_loss(self, states, actions, rewards, next_states, dones, preferences, targets, advantages):
        with torch.no_grad():
            ...
            target_batch = self.normalize_y(target_batch) ### 最后来标准化 target_y ###
            adv_batch = self.normalize_y(adv_batch)       ### 最后来标准化 target_y ###
            ################################################################################
            ################################################################################
        ...
        # Actor loss
        actor_loss = -m.log_prob(action_batch) * wadv_batch.detach() ############### if stop?????????? ###
    ...
    def train(self):
        ...
        for episode in pbar:
            ...
            if len(batch_datas['states']) > Config.BATCH_SIZE: ### if update then begin update ###
                ...
                if len(infos) % 50 == 0:
                    np.savetxt('res/final_popart_infos.txt', np.array(infos), fmt='%.3f')
                    torch.save(self.net, 'models/PopArtEnvMoFianl.pkl')
